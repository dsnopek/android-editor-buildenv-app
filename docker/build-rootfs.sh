...
docker build "$SCRIPT_DIR" --platform=linux/arm64/v8 -f "$SCRIPT_DIR/Dockerfile.godot" -t $IMAGE_NAME \
	--build-arg="JDK_VERSION=17" --build-arg="BUILD_TOOLS=35.0.0" --build-arg="TARGET_SDK=35"

CONTAINER_ID=$(docker create --platform=linux/arm64/v8 $IMAGE_NAME)
